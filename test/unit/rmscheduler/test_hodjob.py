...
class HodRMSchedulerHodjobTestCase(unittest.TestCase):
    ...
    def test_hodjob_set_type_class(self):
        ...
        with patch('hod.rmscheduler.hodjob.HodJob.get_hod', side_effect=lambda: ('sentinel1', 'sentinel2')):
            hj = hrh.HodJob(opt)
            hj.set_type_class()
        assert hj.type_class == ResourceManagerScheduler
    ...
    def test_mympurunhod_generate_exe(self):
        ...
        assert exe[0] == 'mympirun --output=$None/hod.output.$None --hybrid=1 --variablesprefix=HADOOP,JAVA,HOD,MAPRED,HDFS,HDFS,MAPRED python sentinel1 --hod-envclass=MympirunHod'
        """ From prod:
