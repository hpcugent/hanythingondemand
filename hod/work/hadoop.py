...
class Hadoop(Work, HadoopOpts):
    ...
    def use_sdp_java(self, intf, sdpdebug=True):
        ...
        ## add default general (java sdp is a bit stupid; doesn't recognize a bind to 0.0.0.0 as valid part for configured bind subnet)
        ...
        refdir = os.path.join('/tmp', 'hod', pwd.getpwuid(os.getuid())[0]) ## is part of basedir, so exists
        ...
        self.log.debug("Written sdp.conf %s with content %s" % (sdpconf_fn, content))
        ...
        if sdpdebug:
            ...
            debuglog = os.path.join("%s" % self.env_params['%s_LOG_DIR' % self.daemonname.upper()], debug_name)
            self.log.debug("Going to log sdp to %s" % debuglog)
            javaopts += Arguments(["-Dcom.sun.sdp.debug=%s" % (debuglog)])
            refjavaopts += Arguments(["-Dcom.sun.sdp.debug=%s/%s" % (refdir, debug_name)])

        self.add_param('mapred.child.java.opts', refjavaopts)
        self.add_param('%s_OPTS' % self.daemonname.upper(), javaopts, is_env=True)
    ...
    def use_sdp_libsdp(self, intf, sdpdebug=True):
        ...
        if sdpdebug:
            loglvl = 7
        debuglog = os.path.join("%s" % self.env_params['%s_LOG_DIR' % self.daemonname.upper()], "sdp.debug.log")
        self.log.debug("Going to log sdp to %s" % debuglog)
        ...
        ldpreload = ':'.join([libsdp_fn] + os.environ.get('LD_PRELOAD', '').split(':'))
        self.log.debug("Added libsdp %s to LD_PRELOAD %s" % (libsdp_fn, ldpreload))
